...
export default function HomeScreen() {
  ...
  const [sortBy, setSortBy] = useState<"recommended" | "newest" | "price_low">(
    "recommended"
  );

  // Get active filters
  const activeCleanFilter = filterChips.find(
    (chip) => chip.id === "clean"
  )?.active;
  const activePriceFilter = filterChips.find(
    (chip) => chip.id === "under-30k"
  )?.active;
        ...
        <View className="flex-row items-center justify-between mb-4">
          <View>
            ...
            <Text className="text-body text-text-muted">
              {user?.firstName ||
                user?.publicMetadata?.displayName?.split(" ")[0] ||
                user?.unsafeMetadata?.displayName?.split(" ")[0] ||
                "there"}
              ! Find your next car
            </Text>
          </View>
